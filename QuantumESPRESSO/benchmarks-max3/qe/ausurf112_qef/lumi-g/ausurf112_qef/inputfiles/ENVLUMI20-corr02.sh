...
# Load CrayEnv to have access to some LUST-installed software
module load CrayEnv
# CrayEnv will set the appropriate target modules for the node type
# where it is used, but as we may want to do cross-compiling, we'll
# we'll load the CPU and GPU target modules anyway.
module load craype-x86-trento
module load craype-accel-amd-gfx90a
# Set the CPE version
module load cpe/25.09
# Load the Cray Programming Environment for the Cray compilers
# Turns out something in cpe/25.09 is broken as this will load 
# the wrong versions of modules...
