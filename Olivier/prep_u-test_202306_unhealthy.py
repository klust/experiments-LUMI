...
for index in range(lower_bound,upper_bound+1):
	...
	with open('unhealthy_unbound'+ s_i +'.slurm','w') as myfile:
		myfile.write( '''#!/usr/bin/env bash

#SBATCH --job-name=unhealthy_unbound_sim''' + s_i  + '''
#SBATCH --nodes=2
...
#SBATCH --nodelist=nid001446,nid001899
...
RUNDIR=$SCRATCH/unhealthy_unbound_sim'''+s_i+'''
