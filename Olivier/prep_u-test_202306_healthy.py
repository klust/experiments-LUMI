...
for index in range(lower_bound,upper_bound+1):
	...
	with open('healthy_unbound'+ s_i +'.slurm','w') as myfile:
		myfile.write( '''#!/usr/bin/env bash

#SBATCH --job-name=healthy_unbound_sim''' + s_i  + '''
#SBATCH --nodes=2
...
#SBATCH --nodelist=nid001952,nid001954
...
RUNDIR=$SCRATCH/healthy_unbound_sim'''+s_i+'''
