...
C      nn= int(ttarget/(cpu/3.0))
C     Number of iterations forced to 50 for the XL case on a 2x2x2 decomposition	
      nn=50
C
C     end the test loop
      if(id .eq. 0) then
...
C
      ...
      DO loop=1,nn
         ...
         wgosa=0.0
#ifdef CRAYPAT
      call PAT_region_begin( 1, "part_1", istat )
#endif
         DO K=2,kmax-1
            DO J=2,jmax-1
               DO I=2,imax-1
...
C     
         ...
         call PAT_region_begin( 2, "part_2", istat )
#endif
