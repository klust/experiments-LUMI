...
main(int argc,char *argv[])
{
 ...
 nn=50; // forced to 50 iterations for the XL case on a 2x2x2 decomposition
...
jacobi(int nn)
{
  ...
  #pragma omp target data map(to: a,b,c,p,wrk2,wrk1,bnd) 
  {

  for(n=0 ; n<nn ; ++n){
    gosa = 0.0;
    wgosa= 0.0;
   #pragma omp target teams distribute reduction(+: wgosa) private(s0,ss)
    for(i=1 ; i<imax-1 ; ++i)
   #pragma omp parallel for reduction (+: wgosa) 
      for(j=1 ; j<jmax-1 ; ++j)
   #pragma omp simd reduction (+: wgosa)
        for(k=1 ; k<kmax-1 ; ++k){
