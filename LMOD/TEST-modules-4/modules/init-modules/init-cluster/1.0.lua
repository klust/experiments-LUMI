...
local TEST_root = myFileName():match( '(.*)/modules/init%-modules/init%-cluster/.*' )
if os.getenv( '_TEST_LMOD_DEBUG' ) ~= nil then
...
--
prepend_path( 'MODULEPATH', pathJoin( TEST_root, 'modules/Stacks' ) )
...
if os.getenv( 'TRIGGER_BUG' ) ~=  nil then
    prepend_path( 'MODULEPATH', pathJoin( TEST_root, 'modules/init-modules' ) )
end
