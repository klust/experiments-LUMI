...
double generateUniformRandom() {
  static unsigned seed = 781;
  static unsigned int x = seed;
  const unsigned long int a = 1664525;
  const unsigned long int c = 1013904223;
  const unsigned long int m = 4294967296;
  x = (a * x + c) % m;
  return static_cast<double>(x) / m;
}

// Define a function that maps a uniform random number to the desired range [a,b)
double uniformReal(double a, double b) {
  return a + (b - a) * generateUniformRandom();
}
...
FreePathResult FreePathSampling(const Ray& ray, Flow& flow)
{
    ...
    res.d = dmin - log(1-uniformReal(0, 1)) / km;
    while (res.d <= res.dmax && ExtinctionCoefficient(res.ftle = flow.SampleFTLE(ray.Origin + res.d * ray.Direction, startTime, endTime, stepSize, Vec3d(separationDistance))) / km < uniformReal(0, 1))
        res.d = res.d - log(1-uniformReal(0, 1)) / km;
    return res;
}
